...
struct PersistenceController {
    ...
    static let preview: PersistenceController = {
        ...
        createPreviewSampleData(in: result.container.viewContext)
        return result
    }()

    /// Creates sample data for SwiftUI previews
    @MainActor
    private static func createPreviewSampleData(in context: NSManagedObjectContext) {
        for i in 0 ..< 5 {
            let imageEval = createSampleImageEvaluation(index: i, in: context)
            let evalResult = createSampleEvaluationResult(index: i, in: context)

            // Link relationships
            evalResult.imageEvaluation = imageEval
            imageEval.currentEvaluation = evalResult
            imageEval.mutableSetValue(forKey: "evaluationHistory").add(evalResult)
        }

        do {
            try context.save()
        } catch {
            fatalError("Preview sample data save failed: \(error)")
        }
    }

    @MainActor
    private static func createSampleImageEvaluation(index i: Int, in context: NSManagedObjectContext) -> ImageEvaluation {
        let imageEval = ImageEvaluation(context: context)
        imageEval.id = UUID()
    ...
    @MainActor
    private static func createSampleEvaluationResult(index i: Int, in context: NSManagedObjectContext) -> EvaluationResult {
        let result = EvaluationResult(context: context)
        result.id = UUID()
